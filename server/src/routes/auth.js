...
const verifyGoogleToken = async (idToken, platform = 'web') => {
  try {
    const client = platform === 'ios' ? iosClient : 
                   platform === 'android' ? androidClient : webClient;
...
// POST /api/auth/google - Sign in with Google (Native - uses ID token)
  ...
  try {
    ...
    if (deviceId) {
      ...
      await pool.execute(
        ...
        [tokenId, user.id, deviceId, refreshToken, deviceName || 'Unknown Device', refreshToken]
      );
    }
...
router.post('/guest', async (req, res) => {
  try {
    ...
    const guestPicture = 'https://ui-avatars.com/api/?name=Guest+User&background=random';
    ...
    if (existingUsers.length > 0) {
      ...
      user = { id: userId, email: guestEmail, name: guestName, avatar_url: guestPicture };
    }
    ...
    if (deviceId) {
       ...
       await pool.execute(
         `INSERT INTO device_tokens (id, user_id, device_id, refresh_token, device_name)
          VALUES (?, ?, ?, ?, ?)
          ON DUPLICATE KEY UPDATE refresh_token = ?, last_used_at = NOW()`,
         [tokenId, user.id, deviceId, refreshToken, deviceName || 'Guest Session', refreshToken]
       );
    }
...
router.post('/google/web', async (req, res) => {
  try {
    const { googleAccessToken, googleUser: gUser, platform, deviceId, deviceName } = req.body;
    ...
    if (existingUsers.length > 0) {
      ...
      user = { id: userId, google_id: googleId, email, name, avatar_url: picture };
    }
    ...
    if (deviceId) {
      ...
      await pool.execute(
        `INSERT INTO device_tokens (id, user_id, device_id, refresh_token, device_name)
         VALUES (?, ?, ?, ?, ?)
         ON DUPLICATE KEY UPDATE refresh_token = ?, last_used_at = NOW()`,
        [tokenId, user.id, deviceId, refreshToken, deviceName || 'Web Browser', refreshToken]
      );
    }
...
router.post('/refresh', async (req, res) => {
  try {
    ...
    const newAccessToken = generateToken(deviceToken.user_id, deviceToken.email);
